...
/** @deprecated Fluxo único — parâmetro ignorado, mantido por compatibilidade de assinatura */
export function getStatusFlowForMeasurementFlow(
  _flow?: MeasurementFlow,
): Partial<Record<OsStatus, OsStatus[]>> {
...
export function canTransitionWithFlow(
  from: OsStatus,
  to: OsStatus,
  _flow?: MeasurementFlow,
): boolean {
...
export function getAdvanceFlow(
  _flow?: MeasurementFlow,
): Partial<Record<OsStatus, OsStatus[]>> {
...
/** @deprecated Fluxo único — labels legados removidos da UI */
export const MEASUREMENT_FLOW_LABELS: Record<MeasurementFlow, string> = {
  cliente_informou: "Medição",
  profissional_mediu: "Medição",
};

/** Etapas visuais do wizard (ordem de execução) */
export function getWizardStepsForFlow(_flow?: MeasurementFlow): OsStatus[] {
  return [
    "medicao_orcamento",
    "medicao_final",
    "cortes",
...
export function getStepIndexForFlow(
  status: OsStatus,
  _flow?: MeasurementFlow,
): number {
